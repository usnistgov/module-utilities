...
def do_meth_test(x, key=None, docstring='a doc string', check_empty=True):
    ...
    del x._cache
    meth_test(x, 'meth', target, args=(3,), kws=dict(y=4), key=key_tot, docstring=docstring)
    assert tuple(x._cache.keys()) == (key_tot,)
...
def test_clear():
    ...
    meth_test(x, meth='meth', value=(1,2,3,4), args=(3, 4), key=key_meth, docstring=docstring)
    ...
    meth_test(x, meth='meth', value=(2,4,3,4), args=(3, 4), key=key_meth, docstring=docstring)
